...
import com.example.entrega1proyecto.listDetails
...
class PostListsToDBlistDetails() :
    ...
    override fun doInBackground(vararg params: List<ListBDD>?): ArrayList<Long>? {
        ...
        params[0]!!.forEach {
            ...
            if (lisBDD == null){
                PostToDBBlistDetails()
                    .execute(it)
            }
            else{
                val listBDD = lisBDD.list
                if (it.updated_at > listBDD.updated_at) {
                    UpdateListsToDBBlistDetails()
                        .execute(it)
                } else if (it.updated_at < listBDD.updated_at) {
                    UpdateListToAPIlistDetails()
                        .execute(listBDD)
                }
            }
            GetItemsFromApilistDetails()
                .execute(it)
        }
        return idFaltantes
    }
                ...
                if (response.isSuccessful) {
                    if (response.body() != null) {
                        if (response.body()!!.isNotEmpty()) {
                            LoadItemsToBDlistDetails()
                                .execute(response.body())
                        }
                        else{
                            listDetails.Companion.GetTheList(
                                activityCominglistDetails
                            )
                                .execute(listID)
                        }
...
class LoadItemsToBDlistDetails() :
    ...
    override fun doInBackground(vararg params: List<ItemBDD>?): ArrayList<Long>? {
        ...
        params[0]!!.forEach {
            ...
            if (iteBDD == null){
                PostItemToDBBlistDetails()
                    .execute(it)
            }
            else{
                if (it.updated_at > iteBDD.updated_at) {
                    UpdateItemToDBBlistDetails()
                        .execute(it)
                } else if (it.updated_at < iteBDD.updated_at) {
                    UpdateItemToAPIlistDetails()
                        .execute(iteBDD)
                }
            }
        }
